...
public class TableListField extends ListField implements ListFieldCallback {
	...
	public static final int[] meritWidth = { constantColWidth * 2,
			constantColWidth * 10, constantColWidth * 4, constantColWidth * 2 };
	...
	public TableListField(ListFieldListener listener, int table, int page) {
		...
		setRowHeight(even_bg.getHeight() + border.getHeight());
			...
			try {
				...
				if (_merit_pos > 0 && _merit_pos <= 5) {
					if (item.merit_playerphotoURL.length() > 0) {
						...
						try {
							Utility.getWebData(item.merit_playerphotoURL,
									new WebDataCallback() {
										public void callback(byte[] data) {
											if (data != null) {
												((DataCentre) _elements
														.elementAt(_index)).merit_playerphoto = data;
											}
										}
									});
						} catch (IOException e) {
					...
					setupProfile(g, item.merit_playerphoto, item.merit_player,
							y, meritWidth[1], true);
				} else {
	...
	private void setupProfile(Graphics g, byte[] playerPhoto,
			String playerName, int y, int compareWidth, boolean standalone) {
		int desiredWidth = 0;
		int desiredHeight = 0;
		...
		if (playerPhoto != null && playerPhoto.length > 2) {
			...
			try {
				playerPhoto_bmp = Bitmap.createBitmapFromBytes(playerPhoto, 0,
						playerPhoto.length, 1);
				desiredWidth = 46;
				desiredHeight = (desiredWidth * playerPhoto_bmp.getHeight())
						/ playerPhoto_bmp.getWidth();
				playerPhoto_bmp = Utility.resizeBitmap(playerPhoto_bmp,
						desiredWidth, desiredHeight);
				g.drawBitmap(text_x, text_y, desiredWidth, desiredHeight,
						playerPhoto_bmp, 0, 0);
			} catch (Exception e) {
			}
		}
		...
		text_x += desiredWidth + padding;// added for the space between
		// playerPhoto and playerName
	...
	private void setupCell(Graphics g, String printText, int y,
			int compareWidth, boolean standalone) {
	...
	private void calculateCellsInbetween(int y, int compareWidth) {
		text_y = y + padding;
		temp_x += compareWidth + padding + padding
				+ padding;
		text_x = temp_x;
	}
