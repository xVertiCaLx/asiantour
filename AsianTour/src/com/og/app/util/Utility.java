...
public class Utility {
	...
	/*
	 * public static ANewsFeed getDefaultPhotoGalleryFeed(){ return new
	 * ANewsFeed (Const.ID_PHOTOGALLERY, Lang.TAB_PHOTOGALLERY,
	 * Const.FEED_PHOTO, "res/tab_photogallery.png", "",
	...
	public static Bitmap resizeBitmap(Bitmap image, int width, int height) {
		// Note from DCC:
		// an int being 4 bytes is large enough for Alpha/Red/Green/Blue in an
		// 8-bit plane...
		// my brain was fried for a little while here because I am used to
		// larger plane sizes for each
	...
	public static String turnOffURLSpecialChar(String str) {
		String[][] HTMLARR = new String[][] { { " ", "%20" }, { "+", "%2B" }, };
		for (int i = 0; i < HTMLARR.length; i++) {
			...
			do {
				...
				if (tmpindex < 0)
					break;
				str = str.substring(0, tmpindex)
						+ HTMLARR[i][1]
						+ str.substring(tmpindex + HTMLARR[i][0].length(), str
								.length());
				index = tmpindex;
			} while (true);
		}
	...
	public static String turnOffHtmlTag(String str) {
		...
		do {
			...
			try {
				...
				if (intno < 256) {
					char ch = (char) intno;
					str = str.substring(0, tmpindex) + ch
							+ str.substring(tmpindex2 + 1, str.length());
